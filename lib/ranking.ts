import type { CreatorRow, RankedRow } from '@/lib/analytics/types'
...
export function rankRows(rows: CreatorRow[]): RankedRow[] {
  ...
  const normalizers = WEIGHTS.map(({ key }) => minMax(rows.map((r) => Number(r[key]) || 0)))
  ...
  const scored = rows.map((row) => {
    ...
    return { ...row, composite_score: Number((score * 100).toFixed(1)) }
  })
